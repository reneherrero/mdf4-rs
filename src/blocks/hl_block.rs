...
use crate::{
    Error, Result,
    blocks::common::{BlockHeader, read_u64, validate_buffer_size},
};

/// Parse an `##HL` block and return the address of the next block in the measurement chain
/// (typically `##DL` or `##DT`).
pub(crate) fn hl_next_block_addr(block_bytes: &[u8]) -> Result<u64> {
    validate_buffer_size(block_bytes, 24)?;
    let header = BlockHeader::from_bytes(&block_bytes[..24])?;
    let total = crate::blocks::common::u64_to_usize(header.length, "##HL length")?;
    ...
    let links_end = 24usize.saturating_add(
        link_count
            .checked_mul(8)
            .ok_or_else(|| Error::BlockSerializationError("##HL: link section overflow".into()))?,
    );
    if links_end > total {
...
pub(crate) fn skip_hierarchy_blocks(mmap: &[u8], mut addr: u64) -> Result<(u64, BlockHeader)> {
    loop {
        ...
        let off = crate::blocks::common::u64_to_usize(addr, "data block link")?;
        ...
        let len = crate::blocks::common::u64_to_usize(header.length, "##HL")?;
        validate_buffer_size(&mmap[off..], len)?;
        addr = hl_next_block_addr(&mmap[off..off + len])?;
    }
}
